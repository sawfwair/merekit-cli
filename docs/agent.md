...
For Business workspace and website work, start with `mere business workspace status --workspace WORKSPACE_ID --json` and `mere business site current --workspace WORKSPACE_ID --json`. Do not use `site create`, `site save-draft`, `site approve`, or `site regenerate` blindly against a preview-ready request. If a preview-ready request needs revision, run `mere business site request-changes --workspace WORKSPACE_ID --json` first. Use `site import-existing` for an existing website, `site bundle ...` for a static bundle generated by Claude or Codex, and `mere dynasite ...` for provider-level inspection or recovery. The full loop is documented in [Business Workspace And Site Lifecycle](/business-site).
