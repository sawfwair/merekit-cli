...
function toolName(entry: RegistryEntry, command: ManifestCommand): string {
...
function inputToArgs(input: ToolInput, command: ManifestCommand): string[] {
	...
	if (input.json ?? command.supportsJson) args.push('--json');
	if (input.data !== undefined) args.push('--data', JSON.stringify(input.data));
	if (input.dataFile) args.push('--data-file', input.dataFile);
	if (input.yes) args.push('--yes');
	if (input.confirm) args.push('--confirm', input.confirm);
	return args;
}
...
function registerToolCompat(server: unknown, spec: ToolSpec, handler: (input: unknown) => Promise<string>): void {
	...
	const callback = async (input: unknown) => ({
		content: [{ type: 'text' as const, text: await handler(input) }]
	});
