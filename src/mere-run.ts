...
export type MereRunSource = 'explicit' | 'path' | 'global' | 'app' | 'cached' | 'source' | 'dmg' | 'missing';
...
function appBundleCandidates(env: NodeJS.ProcessEnv): string[] {
...
async function usableCli(bin: string, env: NodeJS.ProcessEnv): Promise<{ ok: boolean; detail?: string }> {
	const result = await runCapture(bin, ['model', 'list'], { env, timeoutMs: 10_000 }).catch((error: unknown) => ({
		code: 124,
		stdout: '',
...
async function inspectCandidate(
	source: Exclude<MereRunSource, 'source' | 'dmg' | 'missing'>,
	bin: string,
	env: NodeJS.ProcessEnv,
	installBin: string,
	sourceDir: string,
	steps: MereRunSetupResult['steps']
): Promise<MereRunSetupResult | null> {
	if (!(await isExecutable(bin))) return null;
...
export async function setupMereRun(options: MereRunSetupOptions): Promise<MereRunSetupResult> {
	...
	if (existing.ok && !options.force && existing.source !== 'app') return existing;
...
export async function pullMereRunModels(options: MereRunSetupOptions, requests: MereRunModelRequest[]): Promise<MereRunModelsResult> {
	...
	for (const pull of pulls) {
		if (pull.skipped || !pull.ok) continue;
