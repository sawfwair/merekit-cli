...
App namespaces:
  business finance dynasite projects agent today zone video network email gives works media deliver link
`;
}
...
type CapturedRootJson = { code: number; stdout: string; stderr: string; value: unknown | null };
...
type BootstrapArtifacts = {
	...
	checks: Record<string, { code: number; ok: boolean; stderr: string }>;
	appsList: CapturedRootJson;
	doctor: CapturedRootJson;
	authStatus: CapturedRootJson;
	financeProfiles: CapturedRootJson;
	context: CapturedRootJson;
	manifest: CapturedRootJson;
	snapshot: CapturedRootJson;
};

async function collectBootstrapArtifacts(
	io: CliIO,
...
function readStringListFlag(flags: Record<string, string | boolean | string[]>, name: string): string[] {
	const value = flags[name];
...
async function runSetup(io: CliIO, action: string | undefined, args: string[], flags: Record<string, string | boolean | string[]>): Promise<number> {
	if (action === 'mere-run') return runSetupMereRun(io, args, flags);
...
function mereRunSetupOptions(paths: ReturnType<typeof resolveMerePaths>, io: CliIO, flags: Record<string, string | boolean | string[]>) {
	return {
		env: io.env,
...
async function runSetupMereRun(io: CliIO, args: string[], flags: Record<string, string | boolean | string[]>): Promise<number> {
	const paths = resolveMerePaths(io.env);
	const started = Date.now();
	const options = mereRunSetupOptions(paths, io, flags);
	if (args[0] === 'models' || (args[0] === 'model' && args[1] === 'pull')) {
		const directModels = args[0] === 'model' && args[1] === 'pull' ? [args[2]].filter((value): value is string => Boolean(value)) : [];
		const app = readStringFlag(flags, 'app');
		...
		if (readBooleanFlag(flags, 'json')) {
			...
			for (const model of result.models) io.stdout(`${model.id}: ${model.status}\n`);
			for (const pull of result.pulls) io.stdout(`${pull.id}: ${pull.skipped ? 'already installed' : pull.ok ? 'pulled' : 'failed'}\n`);
	...
	if (readBooleanFlag(flags, 'json')) {
		...
		if (result.version) io.stdout(`version: ${result.version}\n`);
	} else {
...
type AuthHealth = {
	ok: boolean;
	status: 'authenticated' | 'unauthenticated' | 'not_required';
	reasons: string[];
	payload: unknown | null;
};

function parseJsonMaybeNull(text: string): unknown | null {
...
function collectExpirationValues(value: unknown, output: string[] = []): string[] {
	if (Array.isArray(value)) {
		for (const item of value) collectExpirationValues(item, output);
		return output;
	}
	...
	for (const [key, nested] of Object.entries(value)) {
		...
		if (isRecord(nested) || Array.isArray(nested)) collectExpirationValues(nested, output);
	}
	return output;
}

function textExpirationValues(text: string): string[] {
	return [...text.matchAll(/\bexpires(?:At|On)?\s*[:=]\s*([0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9:.+-]+Z?)/giu)]
		.map((match) => match[1])
		.filter((value): value is string => Boolean(value));
}

function expiredValues(values: string[], now = Date.now()): string[] {
...
function authHealthForResult(entry: RegistryEntry, result: ProcessResult): AuthHealth {
	...
	if (isRecord(payload)) {
		if (payload.ok === false) reasons.push('ok_false');
		if (payload.authenticated === false) reasons.push('authenticated_false');
		if (payload.authenticated === true && payload.user === null) reasons.push('authenticated_user_null');
		...
		if (expired.length > 0) reasons.push(`expired:${expired[0]}`);
	} else {
...
async function financeAuthStatus(
	...
	whoami: { ok: boolean; code: number; stdout: unknown; stderr: string; authStatus: AuthHealth['status']; authReasons: string[] };
}> {
...
async function runFinanceProfiles(
	...
	if (action === 'login') {
		const name = args[0]?.trim();
		if (!name) throw new Error('Usage: mere finance profiles login NAME [--base-url URL] [--json]');
		const baseUrl = readStringFlag(flags, 'base-url');
		const existing = config.profiles[name];
		const nextBaseUrl = baseUrl ?? existing?.baseUrl;
		if (!nextBaseUrl?.trim()) {
		...
		if (!entry) throw new Error('Finance app is not registered.');
		const result = await delegateToApp(
			io,
			entry,
			['auth', 'login'],
			{ 'base-url': nextBaseUrl, profile: name, json: readBooleanFlag(flags, 'json') },
			{ capture: true }
		);
		const payload = {
			ok: result.code === 0,
...
async function runAuth(io: CliIO, action: string | undefined, flags: Record<string, string | boolean | string[]>): Promise<number> {
	...
	for (const entry of entries) {
		...
		if (requestedWorkspace && authFlags.workspace === undefined) authFlags.workspace = requestedWorkspace;
...
type ReadOnlyAuditCoverage = {
	...
	skippedReadCommands: Array<{ command: string[]; reason: string }>;
};

type SelectorHints = {
	workspace: string | null;
	inferredTenants: Array<{ app: string; value: string; sourceCommand: string[] }>;
	inferredStores: Array<{ app: string; value: string; sourceCommand: string[] }>;
	missing: Array<{ app: string; selector: string; discoveryCommand: string[]; reason: string }>;
};
...
type ReadOnlyAuditProgress = {
	start(totalApps: number, workspace: string | undefined): void;
	appStart(app: string, index: number, total: number): void;
	selectorStart(app: string): void;
	commandStart(app: string, command: string[]): void;
	commandEnd(row: ReadOnlyAuditCommand): void;
	appEnd(app: ReadOnlyAuditApp): void;
	stop(): void;
};

type ReadOnlyAuditOptions = {
	progress?: ReadOnlyAuditProgress | undefined;
};

function supportsAnsi(io: CliIO): boolean {
	return io.env.NO_COLOR === undefined && io.env.TERM !== 'dumb';
}

function colorize(io: CliIO, color: 'dim' | 'green' | 'red' | 'yellow' | 'cyan', text: string): string {
	if (!supportsAnsi(io)) return text;
	const codes = {
		dim: 2,
		green: 32,
		red: 31,
...
function shouldRenderSnapshotProgress(io: CliIO, flags: Record<string, string | boolean | string[]>): boolean {
	if (readBooleanFlag(flags, 'json')) return false;
	if (io.env.CI === 'true' || io.env.MERE_PROGRESS === '0') return false;
	return io.env.MERE_PROGRESS === '1' || process.stderr.isTTY;
}

function createSnapshotProgress(io: CliIO, flags: Record<string, string | boolean | string[]>): ReadOnlyAuditProgress | undefined {
	if (!shouldRenderSnapshotProgress(io, flags)) return undefined;
	const frames = ['-', '\\', '|', '/'];
	let frame = 0;
	...
	const line = (symbol: string, color: 'green' | 'red' | 'yellow' | 'cyan' | 'dim', label: string) => {
		clearTimer();
	...
	return {
		...
		commandEnd(row) {
			...
			if (row.ok) line('ok', 'green', name);
			else line('!!', 'red', `${name}${row.code === null ? '' : ` exited ${row.code}`}`);
		},
		appEnd(app) {
			if (!app.manifestOk) {
		...
		stop() {
			clearTimer();
			if (activeLabel) process.stderr.write('\r\x1b[2K');
			activeLabel = '';
		}
	};
}

function formatWorkspaceSnapshot(payload: { generatedAt: string; workspace: string; apps: ReadOnlyAuditApp[] }): string {
	...
	if (failingApps.length === 0 && missingSelectors.length === 0) {
		...
		if (failingApps.length > 8) lines.push(`- ${failingApps.length - 8} more app${failingApps.length - 8 === 1 ? '' : 's'} need attention`);
	}
	...
	if (missingSelectors.length > 0) {
		...
		if (missingSelectors.length > 6) lines.push(`- ${missingSelectors.length - 6} more selector hint${missingSelectors.length - 6 === 1 ? '' : 's'}`);
	}
...
async function selectorHintsForEntry(io: CliIO, entry: RegistryEntry, workspace: string | undefined): Promise<SelectorHints> {
...
async function opsWorkspaceSnapshot(io: CliIO, flags: Record<string, string | boolean | string[]>): Promise<number> {
	...
	let payload: Awaited<ReturnType<typeof collectReadOnlyAuditDefaults>>;
	try {
	...
	else writeText(io, formatWorkspaceSnapshot(snapshot).trimEnd());
	return payload.apps.every((app) => app.ok) ? 0 : 1;
}

type OnboardingSeverity = 'info' | 'warning' | 'error';
type OnboardingStatus = 'ready' | 'needs_attention' | 'blocked';
...
function appRecord(records: Record<string, unknown>[], app: string): Record<string, unknown> | null {
...
function manifestCommands(manifest: Record<string, unknown> | null): ManifestCommand[] {
	if (!manifest || !Array.isArray(manifest.commands)) return [];
	return manifest.commands.filter(isRecord) as ManifestCommand[];
}

function globalFlags(manifest: Record<string, unknown> | null): string[] {
	return manifest && Array.isArray(manifest.globalFlags) ? manifest.globalFlags.filter((flag): flag is string => typeof flag === 'string') : [];
}

function safeCommand(entry: RegistryEntry, manifest: Record<string, unknown> | null, command: ManifestCommand, workspace: string | null): string {
	const parts = ['mere', entry.key, ...command.path];
	const flags = globalFlags(manifest);
	if (workspace && flags.includes('workspace')) parts.push('--workspace', workspace);
	if (command.supportsJson) parts.push('--json');
	return commandString(parts);
}

function authOkFor(entry: RegistryEntry, auth: Record<string, unknown> | null): boolean {
	if (entry.authKind === 'none') return true;
...
function financeLoginCommand(flags: Record<string, string | boolean | string[]>): string {
...
function browserLoginCommand(entry: RegistryEntry, workspace: string | null): string {
	return commandString(['mere', 'auth', 'login', '--app', entry.key, ...(workspace ? ['--workspace', workspace] : []), '--json']);
}

function buildOnboardingMarkdown(report: Record<string, unknown>): string {
...
function renderOnboardingText(report: Record<string, unknown>): string {
	const apps = Array.isArray(report.apps) ? report.apps.filter(isRecord) : [];
	const remediation = Array.isArray(report.remediation) ? report.remediation.filter(isRecord) : [];
	...
	if (remediation.length === 0) {
		...
		for (const item of remediation) {
			if (typeof item.nextCommand === 'string') lines.push(`- ${item.nextCommand}`);
		}
	}
	return `${lines.join('\n')}\n`;
}
...
function buildOnboardingReport(
	...
	const missingMereRunModels = Array.isArray(mereRunModels?.missing) ? mereRunModels.missing.filter((item): item is string => typeof item === 'string') : [];
	const workspace = artifacts.workspace;
	const remediation: OnboardingRemediation[] = [];
	const apps: OnboardingAppReport[] = [];
	const selectors: SelectorHints = { workspace, inferredTenants: [], inferredStores: [], missing: [] };
	...
	for (const entry of entries) {
		...
		if (snapshot && Array.isArray(snapshot.commands)) {
			...
			if (failed.length > 0) warnings.push(`${failed.length} safe read command failed.`);
		}
		if (snapshot && isRecord(snapshot.selectorHints)) {
			const hints = snapshot.selectorHints as SelectorHints;
			...
			if (Array.isArray(hints.missing) && hints.missing.length > 0) warnings.push('Some selectors could not be inferred.');
		}

		const readinessScore = Math.max(0, 100 - blockers.length * 35 - warnings.length * 15);
		const status: OnboardingStatus = blockers.length > 0 ? 'blocked' : warnings.length > 0 ? 'needs_attention' : 'ready';
		apps.push({
			app: entry.key,
			label: entry.label,
			status,
...
async function runOnboard(io: CliIO, action: string | undefined, flags: Record<string, string | boolean | string[]>): Promise<number> {
	if (action && action !== 'help') throw new Error('Usage: mere onboard [--interactive] [--waitlist-email EMAIL|--invite-code CODE|--workspace ID] [--app APP] [--target codex|claude] [--output DIR] [--json]');
	if (action === 'help') {
	...
	const target = readStringFlag(flags, 'target') ?? 'codex';
	if (target !== 'codex' && target !== 'claude') throw new Error('Onboard target must be codex or claude.');
	const paths = resolveMerePaths(io.env);
	...
	if (readBooleanFlag(flags, 'json')) writeJson(io, report);
	else io.stdout(renderOnboardingText(report));
	return 0;
}

async function runTui(io: CliIO, flags: Record<string, string | boolean | string[]>): Promise<number> {
	const paths = resolveMerePaths(io.env);
	const registry = createRegistry(paths.mereRoot, paths.packageRoot);
...
async function completionCommandMap(registry: RegistryEntry[], env: NodeJS.ProcessEnv): Promise<Record<string, string[]>> {
...
export async function runCli(argv: string[], io: CliIO): Promise<number> {
	try {
		...
		if (rest[0] === 'skills') return await runSkills(io, rest[1], rest.slice(2), parsed.flags);
