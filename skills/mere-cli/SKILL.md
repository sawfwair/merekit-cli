...
2. For a human first run, prefer `mere tui`. A normal user starts with either a waitlist email or an invite code.
3. If the user does not have an invite yet, run `mere business waitlist join --email EMAIL` or use `mere tui --waitlist-email EMAIL`. This opens a Turnstile and magic-link protected browser handoff; the CLI does not post the email directly.
4. If a human user has an invite code and you are operating headlessly, run `mere business onboard start INVITE_CODE --json` first. This signs in or signs up through the browser if needed and bootstraps the workspace from the invite. It does not require a prior `auth login`.
5. If an agent is creating a workspace from an invite code, require a persistent AgentsIdentify identity first. Run `AGENTSIDENTIFY_API_KEY=ai_... mere business onboard agent-start INVITE_CODE --agent-id STABLE_AGENT_ID --name NAME --slug SLUG --json`. Business mints a short-lived AgentsIdentify proof, consumes the invite, and stores the Mere-owned identity binding. Do not ask for or distribute `MERE_BUSINESS_AGENT_BOOTSTRAP_TOKEN`.
6. If an agent is entering an existing workspace, reissue Business first with `AGENTSIDENTIFY_API_KEY=ai_... mere business auth agent-login --workspace WORKSPACE_ID --agent-id STABLE_AGENT_ID --json`; product app `auth agent-login` commands exchange from that local Business session.
7. After invite bootstrap or session reissue returns a workspace, run `mere onboard --workspace WORKSPACE_ID --target codex --json`. If an operator supplied a workspace ID at the start, treat that as a support/agent path for an already-provisioned workspace. Onboarding writes `AGENT.md`, manifests, auth status, doctor output, workspace snapshot, MCP config, a command reference, `onboarding-report.json`, and `ONBOARDING.md` under `~/.config/mere/agents/default`.
8. Run `mere apps list --json` to see registered apps and CLI locations.
9. Run `mere ops doctor --json` before cross-stack work.
10. Run `mere auth status --all --json` to inspect app session state.
11. Use `mere auth status --app finance --json` and `mere finance profiles list --json` when Finance token/profile state matters.
12. Set or pass a workspace: `mere context set-workspace --workspace WORKSPACE_ID` or use explicit `--workspace`.
13. Use `mere ops workspace-snapshot --workspace WORKSPACE_ID --json` for grouped read-only cross-app inspection.
14. Use `mere apps manifest --app APP --json` to discover exact command paths, risk, flags, JSON/data support, and guardrails.
15. Use `mere <app> ...` for product-owned operations.
16. For agents, start MCP with `mere mcp serve`; add `--allow-writes` only when write tools are intentionally in scope.
...
AGENTSIDENTIFY_API_KEY=ai_... mere business onboard agent-start INVITE_CODE --agent-id STABLE_AGENT_ID --name "Acme Plumbing" --slug acme-plumbing --json
AGENTSIDENTIFY_API_KEY=ai_... mere business auth agent-login --workspace ws_123 --agent-id STABLE_AGENT_ID --json
