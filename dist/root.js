        ...
        for (const command of app.commands) {
            ...
            const required = [
                ...(Array.isArray(command.positionals)
                    ? command.positionals.filter((value) => typeof value === 'string').map((value) => `<${value}>`)
                    : []),
                ...(Array.isArray(command.requiredFlags)
                    ? command.requiredFlags.filter((value) => typeof value === 'string').map((value) => `--${value}`)
                    : []),
                ...(Array.isArray(command.requiredFlagGroups)
                    ? command.requiredFlagGroups
                        .filter((group) => Array.isArray(group))
                        .map((group) => `one of ${group.filter((value) => typeof value === 'string').map((value) => `--${value}`).join(', ')}`)
                    : [])
            ];
            const options = Array.isArray(command.options)
                ? command.options
                    ...
                    const enumValues = Array.isArray(option.enum)
                        ? option.enum.filter((value) => typeof value === 'string').join(', ')
                        : '';
...
function splitPassthroughFlags(flags, supported, leadingNames) {
    const leading = {};
    const trailing = {};
...
export async function delegateToApp(io, entry, appArgs, passthroughFlags, options = {}) {
    ...
    if (effectiveFlags.workspace === undefined && supported.has('workspace')) {
        ...
        if (state.defaultWorkspace)
            effectiveFlags.workspace = state.defaultWorkspace;
    }
...
function parseFinanceConfig(value) {
    const parsed = financeConfigSchema.safeParse(value);
    return parsed.success ? parsed.data : {};
}
function defaultFinanceProfile(env) {
    return {
...
function financeProfileOrDefault(config, name, env) {
...
async function loadFinanceConfig(env) {
    try {
        ...
        if (!profiles.default)
            profiles.default = defaultFinanceProfile(env);
...
async function probeAppAuth(io, entry, flags) {
    if (entry.authKind === 'none') {
    ...
    const result = await delegateToApp(io, entry, command, { ...flags, json: true }, { capture: true }).catch((error) => ({
        code: 1,
        signal: null,
        stdout: '',
...
async function runAuth(io, action, flags) {
    ...
    for (const entry of entries) {
        ...
        const result = probe
            ? probe.result
            : await delegateToApp(io, entry, ['auth', action], authFlags, { capture: true });
...
async function bundledAdapterProvenance(packageRoot) {
    try {
        const manifest = parseJson(await readFile(path.join(packageRoot, 'adapters', 'manifest.json'), 'utf8'));
        if (!isRecord(manifest) || !Array.isArray(manifest.adapters))
            return new Map();
        const manifestSchemaVersion = typeof manifest.schemaVersion === 'number' ? manifest.schemaVersion : null;
        const manifestMode = typeof manifest.mode === 'string' ? manifest.mode : null;
        return new Map(manifest.adapters.filter(isRecord).flatMap((adapter) => {
            if (typeof adapter.app !== 'string')
                return [];
            ...
            return [
                [
                    adapter.app,
                    {
                        ...
                        sourceRepository: typeof source?.canonicalRepository === 'string'
                        ...
                        canonicalRemote: typeof source?.canonicalRemote === 'boolean' ? source.canonicalRemote : null,
                        remoteHeadEvidence: typeof source?.remoteHeadEvidence === 'string'
...
async function opsDoctor(io, flags) {
    ...
    if (readBooleanFlag(flags, 'json'))
        ...
        io.stdout(apps
            .map((app) => `${app.app}: ${app.cliExists && app.versionOk && app.completionOk && app.manifestOk && app.authOk ? 'ok' : 'needs attention'} (${app.authStatus})`)
            .join('\n') + '\n');
    return apps.every((app) => app.cliExists && app.versionOk && app.completionOk && app.manifestOk && app.authOk)
        ? 0
        : 1;
}
...
async function collectReadOnlyAuditDefaults(io, flags, workspace, options = {}) {
    ...
    for (const [index, entry] of entries.entries()) {
        ...
        const authProbe = await probeAppAuth(io, entry, workspace ? { workspace } : {});
        app.auth = {
...
async function opsAudit(io, flags) {
    const paths = resolveMerePaths(io.env);
    const state = await loadState(paths);
    const payload = await collectReadOnlyAuditDefaults(io, flags, readStringFlag(flags, 'workspace') ?? state.defaultWorkspace ?? undefined);
...
export async function runCli(argv, io) {
    try {
        ...
        if (rest[0] === 'docs')
            return await runDocs(io, parsed.positionals[1], parsed.positionals.slice(2), parsed.flags);
