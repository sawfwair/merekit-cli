...
import { OPERATOR_CAPABILITIES, capabilitiesFromFlag, evaluateOperatorPolicy, formatOperator, policyBootstrapGuidance, policyOverrideRequested, policyTaxonomy, resolveOperatorIdentity } from './policy.js';
...
async function loadOptionalConfig(flags) {
    try {
        return (await loadConfig(flags)).config;
    }
    catch (error) {
        if (error instanceof Error && error.message.startsWith('Config not found:') && !stringFlag(flags, 'config') && !process.env.MERE_LINK_CONFIG?.trim())
            return undefined;
        throw error;
    }
}
function requireOperatorCapabilities(config, flags, entityRef, projectRef, capabilities) {
    const operator = resolveOperatorIdentity(config, flags);
    const { policy, decision } = evaluateOperatorPolicy({
        config,
        entityRef,
    ...
    if (policy.source === 'default' && policy.rules.length === 0 && policy.notes.length === 0)
        return;
    if (decision.allowed)
        return;
...
function requireOperatorCapabilitiesForTargets(config, flags, targets, capabilities) {
    const uniqueTargets = new Map();
    for (const target of targets)
        uniqueTargets.set(`${target.entity}/${target.project}`, target);
