...
function broadPolicySurface(entry) {
...
function broadInvocationSurface(entry, toolId) {
    if (entry.surface.kind === 'tool')
        return toolPatternMatches(entry.surface.id, toolId);
    return entry.surface.kind === 'namespace' || entry.surface.kind === 'source';
}
function localAiModelDiscoveryRead(entry, toolId) {
    return (entry.namespace === 'localai' &&
        ['source', 'namespace'].includes(entry.surface.kind) &&
        (toolId === 'localai.models.list' || toolId === 'localai.models.retrieve'));
}
function guardedEntryMatches(entry, toolId, args) {
    if (localAiModelDiscoveryRead(entry, toolId))
        return true;
    const guard = resourceGuard(entry);
    if (guard)
        return resourceGuardMatches(args, guard);
...
function uniqueTargets(entries) {
    ...
    for (const entry of entries)
        targets.set(entry.path, targetForEntry(entry));
...
function namespaceForTool(toolId) {
    return toolId.split('.')[0] ?? '';
}
function entriesForTool(config, toolId) {
...
function readableEntries(config, toolId, args) {
...
function writableEntries(config, toolId, args) {
    return entriesForTool(config, toolId).filter((entry) => (executorWritePatternsForSurfacePolicy(entry.namespace, entry.surface).some((pattern) => toolPatternMatches(pattern, toolId)) &&
...
export function assertExecutorInvocationAllowed(config, mode, toolId, args, apply) {
    if (mode === 'read') {
        ...
        if (matches.length === 0)
            throw new Error(`Executor read denied. Arguments for ${toolId} do not match any declared readable Link surface.`);
